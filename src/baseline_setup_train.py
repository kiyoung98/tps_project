...
class FlowNetAgent:
    ...
    def sample(self, args, mds, std):
        ...
        for s in tqdm(range(args.num_steps)):
            ...
            mds.step(
                1000 * action
            )  # 1000 corresponds to global parameter k of the prior work
        ...
        log_target_reward = -0.5 * torch.square((pd - target_pd) / args.sigma).mean(
            (1, 2)
        )
        ...
        last_idx = args.num_steps * torch.ones(
            args.num_samples, dtype=torch.long, device=args.device
        )  # For the fair comparisions, we train on reward for fixed length and take final index as 500.
...
parser.add_argument(
    ...
    default=0.1,
    type=float,
    help="Approximate standard deviation of Langevin integrator used in https://github.com/LarsHoldijk/SOCTransitionPaths",
)
...
parser.add_argument(
    "--num_rollouts", default=10000, type=int, help="Number of rollouts (or sampling)"
)
parser.add_argument(
...
parser.add_argument(
    ...
    default=0.1,
    type=float,
    help="Final standard deviation of annealing schedule",
)
parser.add_argument(
    "--start_std",
    default=0.2,
    type=float,
    help="Initial standard deviation of annealing schedule",
)
parser.add_argument(
    "--trains_per_rollout",
    default=2000,
